...
create_backup() {
	...
	[ -e "$CWD/.backup/$destination_name" ] &&
		delete "$CWD/.backup/$destination_name" ||
		return 0

	[ -w "$origin_path" ] &&
		mv "$origin_path" "$CWD/.backup/$destination_name" ||
		sudo mv "$origin_path" "$CWD/.backup/$destination_name"
}
...
update_keyboard_layout() {
	case "$1" in
		local)
			...
			create_backup '/usr/share/X11/xkb/symbols/faber' 'keyboard_layout'
		...
		repository)
			print_headline_and_difference 'keyboard layout' '/usr/share/X11/xkb/symbols/faber' "$CWD/keyboard_layout" || return 0

			is_true "$2" ||
				get_permission 'do you want to update the keyboard layout inside the repository [Y/n]? ' ||
				return 0

			rm "$CWD/keyboard_layout"
			cp '/usr/share/X11/xkb/symbols/faber' "$CWD/keyboard_layout"
			;;
	esac
}
